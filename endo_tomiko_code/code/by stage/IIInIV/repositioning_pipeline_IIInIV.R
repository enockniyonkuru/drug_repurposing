...
load('~/Desktop/endometriosis/code/cmap data/cmap_signatures.RData') ## (TO)
gene_list <- subset(cmap_signatures,select=1)
cmap_signatures <- cmap_signatures[,2:ncol(cmap_signatures)] 


# Dataset: Endometriosis
dataset <- "endo"
##dz_signature <- read.csv("~/Desktop/endometriosis/code/by stage/IIInIV/rawdata.csv")
dz_signature <- read.csv("~/Desktop/endometriosis_20220915/code/by stage/IIInIV/Stages III-IV vs Control_Dan.csv") ## (TO)
colnames(dz_signature)[c(1,2)] <- c("GeneID","log2FoldChange") #Relabel columns: estimate is log2FC

dz_signature <- dz_signature[which(dz_signature$adj.P.Val < 0.0001),]
...
N_PERMUTATIONS <- 1000 #default 100000
...
write.csv(rand_cmap_scores,"~/Desktop/endometriosis/code/by stage/IIInIV/rand_cmap_scores.csv",row.names=TRUE)
...
subset_comparison_id <- paste0("InII_", dataset)
analysis_id <- "cmap"
...
save(results, file = "~/Desktop/endometriosis/code/by stage/IIInIV/results.RData") ## (TO)
