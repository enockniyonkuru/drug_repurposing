#####################################################################
# COMPLETE END-TO-END REPLICATION: ALL 5 SIGNATURES
# Using CORRECT rawdata.csv (filtered) for each signature
#####################################################################
...
cat("\n")
cat("════════════════════════════════════════════════════════════════\n")
cat("END-TO-END PIPELINE: ALL 5 SIGNATURES FROM RAW DATA\n")
cat("════════════════════════════════════════════════════════════════\n\n")

# Define all signatures with their correct raw data paths
signatures <- list(
  list(name = "ESE", 
       rawdata_path = "~/Desktop/endo_tomiko_code/code/by phase/ESE/rawdata.csv",
       output_dir = "~/Desktop/endo_tomiko_code/replication/end_to_end_ESE"),
  list(name = "MSE", 
       rawdata_path = "~/Desktop/endo_tomiko_code/code/by phase/MSE/rawdata.csv",
       output_dir = "~/Desktop/endo_tomiko_code/replication/end_to_end_MSE"),
  list(name = "PE", 
       rawdata_path = "~/Desktop/endo_tomiko_code/code/by phase/PE/rawdata.csv",
       output_dir = "~/Desktop/endo_tomiko_code/replication/end_to_end_PE"),
  list(name = "IIInIV", 
       rawdata_path = "~/Desktop/endo_tomiko_code/code/by stage/IIInIV/rawdata.csv",
       output_dir = "~/Desktop/endo_tomiko_code/replication/end_to_end_IIInIV"),
  list(name = "InII", 
       rawdata_path = "~/Desktop/endo_tomiko_code/code/by stage/InII/rawdata.csv",
       output_dir = "~/Desktop/endo_tomiko_code/replication/end_to_end_InII")
)
...
for (sig in signatures) {
  ...
  cat("   - Filtered genes:", nrow(dz_signature), "\n\n")
  ...
  output_file <- file.path(sig$output_dir, paste0("drug_instances_", sig$name, "_from_raw.csv"))
  ...
  results_summary <- rbind(results_summary, 
                          data.frame(Signature = sig$name, 
                                    ...
                                    Top_Score = drug_instances$cmap_score[1]))
}
