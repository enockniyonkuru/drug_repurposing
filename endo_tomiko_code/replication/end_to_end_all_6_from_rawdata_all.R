#####################################################################
# COMPLETE END-TO-END REPLICATION: ALL 6 SIGNATURES FROM RAW DATA
# Using rawdata_all.csv (unfiltered raw data) for all signatures
#####################################################################
...
# Base path
base_path <- "/Users/enockniyonkuru/Desktop/drug_repurposing/endo_tomiko_code"

cat("\n")
cat("════════════════════════════════════════════════════════════════\n")
cat("END-TO-END PIPELINE: ALL 6 SIGNATURES FROM RAW DATA\n")
cat("Using rawdata_all.csv for all signatures\n")
cat("════════════════════════════════════════════════════════════════\n\n")

# Define all 6 signatures with their raw data paths
signatures <- list(
  list(name = "Unstratified", 
       rawdata_path = file.path(base_path, "code/unstratified/rawdata_all.csv"),
       output_dir = file.path(base_path, "replication/end_to_end_all6/Unstratified")),
  list(name = "ESE", 
       rawdata_path = file.path(base_path, "code/by phase/ESE/rawdata_all.csv"),
       output_dir = file.path(base_path, "replication/end_to_end_all6/ESE")),
  list(name = "MSE", 
       rawdata_path = file.path(base_path, "code/by phase/MSE/rawdata_all.csv"),
       output_dir = file.path(base_path, "replication/end_to_end_all6/MSE")),
  list(name = "PE", 
       rawdata_path = file.path(base_path, "code/by phase/PE/rawdata_all.csv"),
       output_dir = file.path(base_path, "replication/end_to_end_all6/PE")),
  list(name = "IIInIV", 
       rawdata_path = file.path(base_path, "code/by stage/IIInIV/rawdata_all.csv"),
       output_dir = file.path(base_path, "replication/end_to_end_all6/IIInIV")),
  list(name = "InII", 
       rawdata_path = file.path(base_path, "code/by stage/InII/rawdata_all.csv"),
       output_dir = file.path(base_path, "replication/end_to_end_all6/InII"))
)

# Create main output directory
dir.create(file.path(base_path, "replication/end_to_end_all6"), showWarnings = FALSE, recursive = TRUE)

# Load CMap data once (same for all signatures)
cat("Loading CMap data...\n")
load(file.path(base_path, "code/cmap data/cmap_signatures.RData"))
gene_list <- subset(cmap_signatures, select=1)
cmap_signatures <- cmap_signatures[, 2:ncol(cmap_signatures)]

cmap_experiments <- read.csv(file.path(base_path, "code/cmap data/cmap_drug_experiments_new.csv"), 
                             stringsAsFactors = FALSE)
valid_instances <- read.csv(file.path(base_path, "code/cmap data/cmap_valid_instances.csv"), 
                            stringsAsFactors = FALSE)
...
# Define cmap_score function (same as original pipeline)
...
for (sig in signatures) {
  ...
  # Step 2: Filter by significance (adj.P.Val < 0.05)
  dz_signature_raw <- dz_signature_raw[which(dz_signature_raw$adj.P.Val < 0.05), ]
  cat("   - After adj.P.Val < 0.05:", nrow(dz_signature_raw), "genes\n")
  
  # Step 3: Filter by fold change (|log2FC| > 1.1)
  dz_signature_raw <- dz_signature_raw[which(abs(dz_signature_raw$log2FoldChange) > 1.1), ]
  cat("   - After |log2FC| > 1.1:", nrow(dz_signature_raw), "genes\n")
  
  # Step 4: Sort and clean gene IDs
  ...
  # Step 5: Keep only genes in CMap
  dz_signature <- dz_signature_raw[which(dz_signature_raw$GeneID %in% gene_list$V1), ]
  cat("   - After intersection with CMap:", nrow(dz_signature), "genes\n\n")
  ...
  write.csv(dz_signature, file.path(sig$output_dir, "dz_signature_processed.csv"), row.names = FALSE)
  
  # Step 6: Compute random distribution (1,000 permutations)
  ...
  # Step 7: Score all drugs
  cat("4. Scoring all", ncol(cmap_signatures), "drug profiles...\n")
  ...
  # Step 8: P-values and q-values
  cat("5. Computing p-values and q-values...\n")
  ...
  # Apply thresholds: q < 0.0001 and cmap_score < 0
  drug_instances <- subset(drug_instances_all, q < 0.0001 & cmap_score < 0)
  cat("   - Significant hits (q < 0.0001 & score < 0):", nrow(drug_instances), "\n")
  
  # Step 10: Deduplicate - keep best score per drug
  cat("7. Deduplicating (keeping best score per drug)...\n")
  ...
  output_file <- file.path(sig$output_dir, paste0("drug_instances_", sig$name, "_from_rawdata_all.csv"))
  write.csv(drug_instances, output_file, row.names = FALSE)
  
  # Also save to main replication folder for easy access
  write.csv(drug_instances, 
            file.path(base_path, "replication", paste0("drug_instances_", sig$name, "_rawdata_all.csv")),
            row.names = FALSE)
  
  # Save full results object
  results <- list(drug_instances_all, dz_signature)
...
write.csv(results_summary, 
          file.path(base_path, "replication/end_to_end_all6/summary_all_6_signatures.csv"),
          row.names = FALSE)

cat("\n✓ ALL 6 SIGNATURES PROCESSED FROM RAW DATA\n")
cat("✓ Output folder:", file.path(base_path, "replication/end_to_end_all6/"), "\n\n")
