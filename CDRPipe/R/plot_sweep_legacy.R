...
#'
#' @param run_dir Directory containing the sweep run outputs
#' @param per_cutoff_df Optional per-cutoff summary data frame
#' @param aggregated_df Aggregated robust-hit data frame
#' @param cutoff_summary Optional cutoff summary data frame
#' @return Logical indicating whether the expected legacy plot files were created
#' @export
#' @importFrom graphics abline barplot grid
...
plot_sweep_outputs_legacy <- function(run_dir, per_cutoff_df = NULL, aggregated_df = NULL, cutoff_summary = NULL) {
  ...
  tryCatch({
    ...
    pl_hist_revsc(sweep_drugs_list, 
                  ...
                  path = img_dir)
    cat("[OK] Generated dist_rev_score.jpeg\n")
  }, error = function(e) {
    cat("[ERROR] Failed to generate dist_rev_score.jpeg:", e$message, "\n")
  })
  ...
  tryCatch({
    ...
    cat("[OK] Generated cmap_score.jpg\n")
  }, error = function(e) {
    cat("[ERROR] Failed to generate cmap_score.jpg:", e$message, "\n")
  })
  ...
  if (!is.null(cutoff_summary) && nrow(cutoff_summary) > 0) {
    tryCatch({
      ...
      dev.off()
      cat("[OK] Generated sweep_cutoff_summary.jpg\n")
    }, error = function(e) {
      cat("[ERROR] Failed to generate sweep_cutoff_summary.jpg:", e$message, "\n")
    })
  } else {
    ...
    tryCatch({
      ...
      dev.off()
      cat("[WARN] Generated placeholder sweep_cutoff_summary.jpg\n")
    }, error = function(e) {
      cat("[ERROR] Failed to generate placeholder sweep_cutoff_summary.jpg:", e$message, "\n")
    })
  }
  ...
  tryCatch({
    ...
    dev.off()
    cat("[OK] Generated comprehensive_sweep_analysis.jpg\n")
  }, error = function(e) {
    cat("[ERROR] Failed to generate comprehensive_sweep_analysis.jpg:", e$message, "\n")
  })
  ...
  if (success) {
    cat("[OK] All 4 legacy plot files created successfully in:", img_dir, "\n")
  } else {
    missing <- expected_files[!expected_files %in% existing_files]
    cat("[WARN] Some files missing:", paste(missing, collapse = ", "), "\n")
  }
