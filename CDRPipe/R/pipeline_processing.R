...
DRP <- R6::R6Class(
  ...
  public = list(
    ...
    probe_id_key         = NULL,    # column containing probe IDs for fallback (e.g., "X")
    probe_id_fallback    = TRUE,    # if TRUE, use probe IDs as Entrez fallback for failed g:Profiler
    ...
    pvalue_method     = "continuous",  # "discrete" or "continuous" p-value calculation
    phipson_smyth_correction = TRUE,   # apply Phipson & Smyth (2010) correction
    ...
    initialize = function(
      ...
      blood_label          = "blood",
      ncores               = NULL,
      pvalue_method        = "continuous",
      phipson_smyth_correction = TRUE
    ) {
      # Original parameters
      ...
      self$probe_id_key         <- probe_id_key
      self$probe_id_fallback    <- probe_id_fallback
      ...
      self$phipson_smyth_correction <- phipson_smyth_correction
    ...
    clean_signature = function(cutoff = NULL) {
      ...
      if (self$combine_log2fc == "average") {
        ...
        # Gene ID handling: two modes
        # 1. gene_conversion_table provided: use custom mapping table
        # 2. Default: use gprofiler2 with probe ID fallback for failed conversions
        
        if (!is.null(self$gene_conversion_table) && file.exists(self$gene_conversion_table)) {
          # Mode 1: Custom gene mapping table (bypass gprofiler2)
          ...
          # Mode 2: Use clean_table with gprofiler2 + probe ID fallback
          cleaned <- clean_table(
            self$dz_signature_raw,  # Use raw data to preserve probe ID column
            ...
            probe_id_key     = self$probe_id_key,
            probe_id_fallback = self$probe_id_fallback
          )
        }
        ...
        for (col in lc_cols) {
          ...
          # Gene ID handling: two modes (same as average mode)
          if (!is.null(self$gene_conversion_table) && file.exists(self$gene_conversion_table)) {
            # Mode 1: Custom gene mapping table
            mapping_tbl <- utils::read.csv(self$gene_conversion_table, sep = '\t', stringsAsFactors = FALSE)
            mapping_tbl <- mapping_tbl[!is.na(mapping_tbl$entrezID), c("Gene_name", "entrezID")]
            mapping_tbl <- mapping_tbl[!duplicated(mapping_tbl), ]
    ...
    score = function() {
      ...
      self$drugs <- query(
        ...
        phipson_smyth_correction = self$phipson_smyth_correction
      )
      invisible(self)
    },
    ...
    run_sweep = function() {
      ...
      if (length(self$sweep_hits) > 0) {
        ...
        tryCatch({
          ...
          self$log("[OK] Sweep analysis plots and consolidated results created successfully")
        }, error = function(e) {
...
#' @param percentile_filtering Optional list controlling percentile-based
#'   filtering in place of a fixed `logfc_cutoff`
