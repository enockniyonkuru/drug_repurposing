"""
Drug Repurposing Visualization: Disease Therapeutic Areas vs Drug Target Classes

Creates innovative visualizations to explore the biological relationships
between disease categories and drug mechanisms in recovered pairs.

Color Convention:
  - TAHOE: Serene Blue (#5DADE2)
  - CMAP: Warm Orange (#F39C12)

Output: figures/ subfolder with multiple visualization types
"""
...
print("Loading data...")
cmap_df = pd.read_csv('open_target_cmap_recovered.csv')
tahoe_df = pd.read_csv('open_target_tahoe_recovered.csv')

print(f"CMAP recovered: {len(cmap_df)} pairs")
print(f"Tahoe recovered: {len(tahoe_df)} pairs")
...
print("\n1. Creating heatmaps...")
cmap_crosstab = create_heatmap(cmap_expanded, "CMAP Recovered Pairs", "heatmap_cmap.png", CMAP_CMAP)
tahoe_crosstab = create_heatmap(tahoe_expanded, "Tahoe Recovered Pairs", "heatmap_tahoe.png", TAHOE_CMAP)
...
def create_comparative_heatmap(cmap_df, tahoe_df, filename):
    ...
    plt.tight_layout()
    plt.savefig('figures/heatmap_comparative_cmap.png', dpi=300, bbox_inches='tight')
    plt.close()
    print(f"  Saved: figures/heatmap_comparative_cmap.png")
    ...
    plt.tight_layout()
    plt.savefig('figures/heatmap_comparative_tahoe.png', dpi=300, bbox_inches='tight')
    plt.close()
    print(f"  Saved: figures/heatmap_comparative_tahoe.png")
...
# =============================================================================
# 3. BUBBLE CHART: Disease-Drug Relationship Strength
# =============================================================================
...
def create_bubble_chart(df, title, filename, cmap_name):
    ...
    for _, row in combo_counts.iterrows():
        if row['count'] >= 5:  # Only label larger bubbles
    ...
    ax.set_xlabel('Drug Target Class', fontsize=12)
    ax.set_ylabel('Disease Therapeutic Area', fontsize=12)
    ax.set_title(f'{title}\nBubble Size & Color = Number of Recovered Pairs', fontsize=14, fontweight='bold')
...
create_bubble_chart(cmap_expanded, "CMAP Recovered Pairs", "bubble_cmap.png", 'YlOrRd')
create_bubble_chart(tahoe_expanded, "Tahoe Recovered Pairs", "bubble_tahoe.png", 'YlGnBu')
...
create_stacked_bar(cmap_expanded, "CMAP Recovered Pairs", "stacked_bar_cmap.png", CMAP_COLOR)
create_stacked_bar(tahoe_expanded, "Tahoe Recovered Pairs", "stacked_bar_tahoe.png", TAHOE_COLOR)
...
def create_radar_chart(cmap_df, tahoe_df, filename):
    ...
    for ta in common_tas:
        ...
        plt.tight_layout()
        plt.savefig(f'figures/radar_{safe_ta_name}.png', dpi=300, bbox_inches='tight')
        plt.close()
...
def create_difference_heatmap(cmap_df, tahoe_df, filename):
    ...
    ax.set_title('Tahoe vs CMAP: Differential Recovery Patterns\n(Blue = Tahoe Higher, Orange = CMAP Higher)', 
...
def create_summary_dashboard(cmap_df, tahoe_df, filename):
    ...
    ax1.set_title('Top Disease Therapeutic Areas\nCMAP vs Tahoe Comparison', fontweight='bold')
    ax1.legend(fontsize=10)
    ax1.invert_yaxis()
    plt.tight_layout()
    plt.savefig('figures/bar_disease_areas.png', dpi=300, bbox_inches='tight')
    plt.close()
    print(f"  Saved: figures/bar_disease_areas.png")
    ...
    ax2.invert_yaxis()
    plt.tight_layout()
    plt.savefig('figures/bar_drug_classes.png', dpi=300, bbox_inches='tight')
    plt.close()
    print(f"  Saved: figures/bar_drug_classes.png")
    ...
    ax3.set_title('CMAP: Drug Target Class Distribution', fontweight='bold', fontsize=14)
    plt.tight_layout()
    plt.savefig('figures/pie_cmap_drug_classes.png', dpi=300, bbox_inches='tight')
    plt.close()
    print(f"  Saved: figures/pie_cmap_drug_classes.png")
    ...
    ax4.set_title('Tahoe: Drug Target Class Distribution', fontweight='bold', fontsize=14)
    plt.tight_layout()
    plt.savefig('figures/pie_tahoe_drug_classes.png', dpi=300, bbox_inches='tight')
    plt.close()
    print(f"  Saved: figures/pie_tahoe_drug_classes.png")
    ...
    ax5.set_title('Coverage: Diseases vs Drugs per Therapeutic Area\nCMAP vs Tahoe', fontweight='bold', fontsize=14)
    ax5.legend(fontsize=10)
    ax5.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig('figures/scatter_coverage.png', dpi=300, bbox_inches='tight')
    plt.close()
    print(f"  Saved: figures/scatter_coverage.png")
    ...
    ax6.set_title('Top Disease → Drug Target Class Combinations', fontweight='bold', fontsize=14)
    ax6.legend(fontsize=10)
    plt.tight_layout()
    plt.savefig('figures/bar_top_combinations.png', dpi=300, bbox_inches='tight')
    plt.close()
    print(f"  Saved: figures/bar_top_combinations.png")
    ...
    Tahoe recovers {len(tahoe_df)/len(cmap_df):.1f}x more pairs
    """
    ...
    plt.suptitle('Drug Repurposing Recovery: CMAP vs Tahoe Dashboard', 
...
# Define distinct colors for disease therapeutic areas and drug target classes
...
def create_chord_style(df, title, filename):
    ...
    - Disease therapeutic areas: Red/Coral
    - Drug target classes: Green
    """
...
# Both use same colors: Red for diseases, Green for drug targets
create_chord_style(cmap_expanded, "CMAP Recovered Pairs", "chord_cmap.png")
create_chord_style(tahoe_expanded, "Tahoe Recovered Pairs", "chord_tahoe.png")
