"""
CREEDS Disease Signature Processing Script

This script processes disease signatures from the CREEDS (CRowd Extracted Expression 
of Differential Signatures) database. It performs the following operations:

1. Extracts ALL diseases directly from the CREEDS JSON database
2. For multi-disease entries (separated by |), creates individual files for each disease
3. Aggregates gene signatures across multiple experiments for each disease
4. Calculates mean and median log-fold changes across experiments
5. Identifies the most comprehensive experiment (highest gene count)
6. Exports individual CSV files for each disease signature
7. Generates detailed statistics reports

Output Structure:
- Individual disease signature CSV files with columns:
  - gene_symbol: Gene identifier
  - logfc_<sig_id>: Log-fold change for each signature/experiment
  - mean_logfc: Mean log-fold change across all experiments
  - median_logfc: Median log-fold change across all experiments
  - common_experiment: Log-fold change from the most comprehensive experiment
  - organism: Source organism (e.g., "human")

Reports Generated:
- Processing statistics saved to reports/ directory
- Includes counts of total, matched, unique diseases
- Organism distribution statistics
