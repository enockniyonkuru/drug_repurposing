...
# Convert to CMap-like format (V1, V2, ...)
cmap_like_genes_filtered = pd.DataFrame({"V1": ranked_genes_x_exper.index})
for j, col in enumerate(tqdm(ranked_genes_x_exper.columns, desc="Converting to CMap-like"), start=2):
    cmap_like_genes_filtered[f"V{j}"] = ranked_genes_x_exper[col].to_numpy()
...
# Convert to CMap-like format (V1, V2, ...)
cmap_like_genes_drugs = pd.DataFrame({"V1": ranked_genes_drugs_df.index})
for j, col in enumerate(tqdm(ranked_genes_drugs_df.columns, desc="Converting to CMap-like"), start=2):
    cmap_like_genes_drugs[f"V{j}"] = ranked_genes_drugs_df[col].to_numpy()
