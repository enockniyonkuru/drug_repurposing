#!/usr/bin/env Rscript
# Batch script to run CREEDS MANUAL disease signatures with both CMAP and TAHOE drug signatures
# This script processes all disease files in the CREEDS manual disease signature directory
# VERSION 2: Checks for existing results and skips if found.

suppressPackageStartupMessages(library(DRpipe))

# Define paths - CREEDS MANUAL
disease_dir <- "../data/disease_signatures/creeds_manual_disease_signatures"


# CMAP configuration 
cmap_signatures <- "../data/drug_signatures/cmap/cmap_genes_filtered.RData"
cmap_meta <- "../data/drug_signatures/cmap/cmap_drug_experiments_new.csv"


# TAHOE configuration - Part 2: genes_filtered (COMMENTED OUT)
tahoe_signatures <- "../data/drug_signatures/tahoe/tahoe_genes_filtered.RData"
...
cat("Found", length(disease_files), "CREEDS MANUAL disease signature files\n\n")
...
batch_report_file <- file.path(report_dir, paste0("creeds_manual_batch_report_", batch_ts, ".txt"))
...
writeLines(paste("Disease source: CREEDS MANUAL"), log_conn)
...
writeLines("CREEDS MANUAL DISEASE SIGNATURES - BATCH ANALYSIS REPORT", report_conn)
writeLines("================================================================================", report_conn)
...
writeLines(paste("  Disease source: CREEDS MANUAL"), report_conn)
...
# --- START MODIFICATION ---
# Get list of existing directories in the output root to check for completed runs
cat("Checking for existing results in:", out_root, "\n")
# We get just the folder names, not the full path
...
for (i in seq_along(disease_files)) {
  ...
  # --- START MODIFICATION ---
  # Check if any existing directory starts with this disease name
  # Use startsWith for exact string matching (no regex escaping needed)
  ...
  if (already_processed) {
    ...
    # Add 'SKIPPED' status to the summary data frame for both CMAP and TAHOE
    ...
    next # This command skips the rest of the loop and moves to the next disease
  }
  # --- END MODIFICATION ---
...
summary_report_file <- file.path(report_dir, paste0("creeds_manual_batch_summary_", batch_ts, ".csv"))
write.csv(results_summary, summary_report_file, row.names = FALSE)

# Print final summary
cat("\n\n")
cat("================================================================================\n")
cat("BATCH RUN COMPLETED - CREEDS MANUAL\n")
cat("================================================================================\n")
cat("Total diseases considered:", length(disease_files), "\n")
...
log_conn <- file(batch_log_file, open = "at")
writeLines("\n================================================================================", log_conn)
writeLines("BATCH RUN COMPLETED - CREEDS MANUAL", log_conn)
writeLines("================================================================================", log_conn)
writeLines(paste("Batch run ended at:", Sys.time()), log_conn)
writeLines(paste("Total diseases considered:", length(disease_files)), log_conn)
...
writeLines(paste("Skipped runs:", sum(results_summary$status == "SKIPPED")), log_conn) # Added skipped count
close(log_conn)
...
writeLines(paste("Total diseases considered:", length(disease_files)), report_conn)
...
writeLines(paste("Skipped runs:", sum(results_summary$status == "SKIPPED")), report_conn) # Added skipped count
writeLines("", report_conn)
