#!/usr/bin/env Rscript
# Batch script to run CREEDS AUTOMATIC disease signatures with both CMAP and TAHOE drug signatures
# This script processes all disease files in the CREEDS automatic disease signature directory

suppressPackageStartupMessages(library(DRpipe))

# Define paths - CREEDS AUTOMATIC
disease_dir <- "../data/disease_signatures/creeds_automatic_disease_signatures"
...
# Common parameters
cmap_valid <- NULL
out_root <- "../results/creeds_automatic_disease_results"
report_dir <- "../reports"
...
cat("Found", length(disease_files), "CREEDS AUTOMATIC disease signature files\n\n")
...
batch_report_file <- file.path(report_dir, paste0("creeds_automatic_batch_report_", batch_ts, ".txt"))
...
writeLines(paste("Disease source: CREEDS AUTOMATIC"), log_conn)
...
writeLines("CREEDS AUTOMATIC DISEASE SIGNATURES - BATCH ANALYSIS REPORT", report_conn)
writeLines("================================================================================", report_conn)
...
writeLines(paste("  Disease source: CREEDS AUTOMATIC"), report_conn)
...
summary_report_file <- file.path(report_dir, paste0("creeds_automatic_batch_summary_", batch_ts, ".csv"))
write.csv(results_summary, summary_report_file, row.names = FALSE)

# Print final summary
cat("\n\n")
cat("================================================================================\n")
cat("BATCH RUN COMPLETED - CREEDS AUTOMATIC\n")
cat("================================================================================\n")
...
log_conn <- file(batch_log_file, open = "at")
writeLines("\n================================================================================", log_conn)
writeLines("BATCH RUN COMPLETED - CREEDS AUTOMATIC", log_conn)
writeLines("================================================================================", log_conn)
