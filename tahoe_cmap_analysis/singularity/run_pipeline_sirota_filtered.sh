#!/bin/bash
### ============================================================
### UCSF Wynton HPC Submission Script — DRpipe: Sirota Lab
### ============================================================
...
#$ -j y
#$ -r y
#$ -N drpipe_sirota           # Job name (unique for this run)
...
### --- Logging setup ---
JOB_NAME=${JOB_NAME:-drpipe_sirota}
...
echo "=============================================================="
echo "🚀 Starting DRpipe Batch Analysis: SIROTA LAB"
echo "Node: $(hostname)"
...
### --- Run the R Script ---
echo "Starting Rscript for SIROTA LAB..."
Rscript ${R_SCRIPT} \
  --disease_dir "../data/disease_signatures/sirota_lab_disease_signatures" \
  --disease_source "SIROTA LAB" \
  --cmap_sig "${CMAP_SIG}" \
  --cmap_meta "${CMAP_META}" \
  --tahoe_sig "${TAHOE_SIG}" \
  --tahoe_meta "${TAHOE_META}" \
  --gene_table "${GENE_TABLE}" \
  --out_root "../results/sirota_lab_disease_results_filtered" \
  --report_dir "${REPORT_DIR}" \
  --report_prefix "sirota_lab"
