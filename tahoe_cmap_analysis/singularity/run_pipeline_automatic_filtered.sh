#!/bin/bash
### ============================================================
### UCSF Wynton HPC Submission Script — DRpipe: CREEDS Automatic
### ============================================================
...
#$ -j y
#$ -r y
#$ -N drpipe_auto             # Job name (unique for this run)
...
echo "=============================================================="
echo "🚀 Starting DRpipe Batch Analysis: CREEDS AUTOMATIC"
echo "Node: $(hostname)"
...
### --- Run the R Script ---
echo "Starting Rscript for CREEDS AUTOMATIC..."
Rscript ${R_SCRIPT} \
  --disease_dir "../data/disease_signatures/creeds_automatic_disease_signatures" \
  --disease_source "CREEDS AUTOMATIC" \
  --cmap_sig "${CMAP_SIG}" \
  --cmap_meta "${CMAP_META}" \
  --tahoe_sig "${TAHOE_SIG}" \
  ...
  --report_prefix "creeds_automatic"
