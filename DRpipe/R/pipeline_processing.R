...
DRP <- R6::R6Class(
  ...
  public = list(
    ...
    clean_signature = function() {
      ...
      self$dz_genes_up   <- dplyr::filter(self$dz_signature,  logFC > 0) |> dplyr::pull(GeneID)
      self$dz_genes_down <- dplyr::filter(self$dz_signature,  logFC < 0) |> dplyr::pull(GeneID)
      self$log("Cleaned signature: n_up=%d  n_down=%d", length(self$dz_genes_up), length(self$dz_genes_down))
      invisible(self)
    },
    ...
    run_all = function(make_plots = TRUE) {
      self$load_cmap()$load_disease()$clean_signature()$score()$annotate_and_filter()
      if (isTRUE(make_plots)) self$quick_report()
      self$save_outputs()
