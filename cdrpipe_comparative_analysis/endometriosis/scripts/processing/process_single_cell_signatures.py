#!/usr/bin/env python3
"""
Process Single-Cell Disease Signatures

Converts and standardizes disease signatures from Single-Cell Excel and CSV files.
Normalizes to consistent format with aggregated statistics for drug discovery.
Handles multi-format input for unified analysis pipeline.
"""
...
def main():
    """
    Main execution function to process Single-Cell signatures.
    """
    # --- Configuration ---
    script_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(os.path.dirname(script_dir))
    
    # FIXED: Use old_single_cell_signatures as source directory
    source_dir = os.path.join(project_root, 'data', 'disease_signatures', 'old_single_cell_signatures')
    
    # New directory for standardized output files
    output_dir = os.path.join(project_root, 'case_study_endomentriosis', 'disease_signatures', 'single_cell_disease_signatures')
    
    # Path to the report file
    reports_dir = os.path.join(project_root, 'case_study_endomentriosis', 'disease_signatures')
    report_path = os.path.join(reports_dir, "single_cell_signatures_report.txt")

    # The Excel file to be processed
    excel_file_path = os.path.join(source_dir, "SingleCellSignature.xlsx")
    ...
    print("="*80)
    print("Single-Cell Signature Processing Script")
    print("="*80)
    ...
    try:
        report_title = "SINGLE-CELL SIGNATURES - PROCESSING REPORT"
