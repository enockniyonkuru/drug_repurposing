#!/usr/bin/env Rscript
#' Run CDRPipe Batch Analysis from Configuration File
#'
#' Reads parameters from a YAML configuration file and executes the CDRPipe
...
resolve_cfg_path <- function(path, repo_root) {
...
parser <- ArgumentParser(description = "Run batch CDRPipe analysis from YAML config file")
...
execution_dir <- dirname(normalizePath(config_file, winslash = "/", mustWork = FALSE))
if (basename(execution_dir) == "batch_configs") {
  execution_dir <- dirname(execution_dir)
}
repo_root <- normalizePath(file.path(execution_dir, "..", ".."), winslash = "/", mustWork = FALSE)
...
tryCatch({
  disease_dir      <- resolve_cfg_path(config$disease$directory, repo_root)
  disease_source   <- config$disease$source
  cmap_signatures  <- resolve_cfg_path(config$cmap$signatures, repo_root)
  cmap_meta        <- resolve_cfg_path(config$cmap$metadata, repo_root)
  tahoe_signatures <- resolve_cfg_path(config$tahoe$signatures, repo_root)
  tahoe_meta       <- resolve_cfg_path(config$tahoe$metadata, repo_root)
  gene_conv_table  <- resolve_cfg_path(config$analysis$gene_table, repo_root)
  ...
  out_root         <- resolve_cfg_path(config$output$root_directory, repo_root)
  report_dir       <- resolve_cfg_path(config$output$report_directory, repo_root)
...
# Build command to call run_cdrpipe_batch.R
cdrpipe_batch_script <- file.path(execution_dir, "run_cdrpipe_batch.R")

# Check if the script exists
if (!file.exists(cdrpipe_batch_script)) {
  cat("Error: run_cdrpipe_batch.R not found at:", cdrpipe_batch_script, "\n")
  quit(status = 1)
}

if (verbose) {
  cat("Using batch script:", cdrpipe_batch_script, "\n\n")
}
...
cmd <- paste("Rscript", cdrpipe_batch_script, paste(cmd_args, collapse = " "))
