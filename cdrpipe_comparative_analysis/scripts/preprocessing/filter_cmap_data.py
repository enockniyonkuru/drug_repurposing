#!/usr/bin/env python3
"""
Filter CMAP Signatures

Filters CMAP drug signatures to shared genes and shared drugs. Creates multiple
RData versions for comparative analysis with Tahoe and for consistent cross-
database drug repurposing experiments.
"""
...
try:
    ...
    print("Error: Could not import 'normalize_drug_name' from 'scripts/preprocessing/utils.py'.")
    exit()
...
PATH_CMAP_SIG = os.path.join(PROJECT_ROOT, "data/drug_signatures/cmap/cmap_signatures.RData")
PATH_CMAP_EXP = os.path.join(PROJECT_ROOT, "data/drug_signatures/cmap/cmap_drug_experiments_new.csv")
PATH_SHARED_GENES = os.path.join(PROJECT_ROOT, "data/shared_genes_cmap_tahoe.csv")
PATH_SHARED_DRUGS = os.path.join(PROJECT_ROOT, "data/shared_drugs_cmap_tahoe.csv")

# Outputs
OUT_DIR_DATA = os.path.join(PROJECT_ROOT, "data/drug_signatures/cmap")
OUT_DIR_REPORTS = os.path.join(PROJECT_ROOT, "data")
