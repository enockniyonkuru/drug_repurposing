#!/usr/bin/env Rscript
#' Tahoe Signatures Part 3: Convert to RData
#'
#' Converts ranked parquet checkpoint to RData format compatible with CDRPipe.
#' Applies chunked processing to manage memory efficiently for large signature
#' matrices before downstream analysis.
#'=
...
log_error <- function(error_msg, context = "", traceback = TRUE) {
  log_message(paste(rep("=", 80), collapse = ""), "ERROR")
  ...
  if (requireNamespace("pryr", quietly = TRUE)) {
    # Note: Using requireNamespace in R, not import/try
...
tryCatch({
  # Define paths
  checkpoint_file <- "../data/drug_signatures/tahoe/checkpoint_ranked_all_genes_all_drugs.parquet"
...
log_message("", "INFO")
log_message(paste(rep("=", 80), collapse = ""))
log_message("HPC PIPELINE PART 3 COMPLETE SUCCESSFULLY", "INFO")
log_message("RESULT: Final RData file saved.")
log_message(paste(rep("=", 80), collapse = ""))
log_message("", "INFO")
