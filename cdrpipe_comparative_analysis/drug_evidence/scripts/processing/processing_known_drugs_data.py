...
Processes Open Targets drug evidence data. Loads disease ontology and known
therapies, generates processed parquet datasets for drug discovery pipeline.
Includes drug phase and status information for candidate prioritization.
"""
...
# (e.g., /.../cdrpipe_comparative_analysis/scripts/processing)
...
INPUT_DATA_DIR = os.path.join(BASE_DIR, 'data/raw_known_drugs')

# Input file paths (now built from BASE_DIR)
DISEASE_FILE_PATH = os.path.join(INPUT_DATA_DIR, 'disease.parquet')
DRUG_FILE_PATHS = [
    os.path.join(INPUT_DATA_DIR, 'part-00000-ecc6ecd3-bcb5-4787-b899-6b8b54085883-c000.snappy.parquet'),
    os.path.join(INPUT_DATA_DIR, 'part-00001-ecc6ecd3-bcb5-4787-b899-6b8b54085883-c000.snappy.parquet')
]

# Output directories (now built from BASE_DIR)
OUTPUT_DATA_DIR = os.path.join(BASE_DIR, 'data/drug_evidence', 'open_targets')
OUTPUT_REPORT_DIR = os.path.join(BASE_DIR, 'reports')

# Output file paths (these are built from the new output dirs)
OUTPUT_DISEASE_FILE = os.path.join(OUTPUT_DATA_DIR, 'disease.parquet')
...
def generate_report(df: pd.DataFrame, rename_map: dict, report_path: str, title: str):
    ...
    try:
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            output_file = report_path.replace('_report.txt', '.parquet').replace('reports', 'data/drug_evidence/open_targets')
