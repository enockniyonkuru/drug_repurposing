#!/usr/bin/env python3
"""
Compare CMAP and Tahoe Datasets

Compares cell lines, drugs, and genes between CMAP and Tahoe signatures.
Generates reports on overlapping datasets and identifies shared experimental
conditions for integrated drug repurposing analysis.

The shared-gene output written by this script:

- `data/shared_genes_cmap_tahoe.csv`

is the preserved gene-universe reference used by the older CREEDS
shared-gene branch under `creeds/data/manual_signatures_shared_genes/`.
"""
...
sys.path.insert(0, os.path.join(PROJECT_ROOT, "scripts", "preprocessing"))

try:
    from utils import normalize_cell_line_name, normalize_drug_name
except ImportError:
    print("Error: Could not import from 'scripts/preprocessing/utils.py'.")
...
PATH_GENE_MAP = os.path.join(PROJECT_ROOT, "data/gene_id_conversion_table.tsv")

# --- Output Report/Data Paths ---
OUT_DIR = os.path.join(PROJECT_ROOT, "data")
FINAL_REPORT_PATH = os.path.join(OUT_DIR, "compare_tahoe_cmap.txt")
PATH_SHARED_CELL_LINES = os.path.join(OUT_DIR, "shared_cell_lines_cmap_tahoe.csv")
PATH_SHARED_DRUGS = os.path.join(OUT_DIR, "shared_drugs_cmap_tahoe.csv")
PATH_SHARED_GENES_FINAL = os.path.join(OUT_DIR, "shared_genes_cmap_tahoe.csv")
